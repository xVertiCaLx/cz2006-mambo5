...
public class CamsListMenuItemForm extends JPanel implements
		JInterfaceController {
	private ArrayList<MenuItem> menuItemList;
	private JPanel menuItemPanel;
	private Map<JButton, MenuItem> menuItemButtons;
	private int menuID, posX = 0, posY = 0, totalWidth = 0, totalHeight = 0,
			currentMenuItem = 0;

	private CamsMainFrame mainFrame;

	public CamsListMenuItemForm(final CamsMainFrame mainFrame,
			final ArrayList<MenuItem> menuItemList, int menuID) {
	...
	public void initMenuItemButtons(int menuID) {
		...
		for (; currentMenuItem < menuItemList.size(); currentMenuItem++) {
			if (menuID == menuItemList.get(currentMenuItem).getMenuID()) {
	...
	public void addMenuItemButtons(final MenuItem menuItem) {
		...
		menuItemButton
				.setBorder(new LineBorder(TITLEBAR_BORDER_COLOUR, 1, true));
		...
		menuItemButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				mainFrame.replacePanel(new CamsEditMenuItemForm(mainFrame,
						menuItem));
			}
		});
		add(menuItemButton);
